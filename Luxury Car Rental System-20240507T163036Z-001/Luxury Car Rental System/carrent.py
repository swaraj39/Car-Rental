...
def addUser():
    ...
    n = udf["User ID"].count()
    udf.loc[n] = [uid, uname, pwd]  # Using loc instead of at
...
def showCharts():
    ...
    if ch == 2:
        ...
        # Assuming "No. of days Booked" represents total days booked for each member
        # Aggregate the data to calculate total days booked per member
        df = df.groupby("M Name")["No. of days Booked"].sum().reset_index()
        df.plot("M Name", "No. of days Booked", kind="bar", color="red")
